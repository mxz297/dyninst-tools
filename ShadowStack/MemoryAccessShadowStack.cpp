...
bool ContainsNoMemoryAccess(BPatch_function* func) {
    ParseAPI::Function* f = ParseAPI::convert(func);
    ...
    for (auto b : f->blocks()) {
        ...
        for (auto iit : insns) {
            ...
            // Filter certain instruction opcodes
            if (i.getOperation().getID() == e_push) continue;
            if (i.getOperation().getID() == e_pop) continue;
            if (i.getOperation().getID() == e_ret_near) continue;
            
            // This instruction contains memory writes.
            // We need to instrument this function
            if (i.writesMemory()) return false;

            // If this instruction touches stack pointer,
            // then it is possible to perform a stack pivoting
            // to cause push instruction to be unsafe
...
int Instrument(BPatch_binaryEdit *app, BPatch_image* image) {
    ...
    for (auto f : *funcs) {
        if (ContainsNoMemoryAccess(f)) continue;
