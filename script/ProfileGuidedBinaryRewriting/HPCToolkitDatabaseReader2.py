...
class HPCToolkitReader:
    ...
    def fill_tables(self):
        ...
        self.metric_data = {}
        profile_db_path = os.path.join(self.dir_name , 'profile1.db')
        cmd = "~/projects/hpctoolkit-dir/install/libexec/hpctoolkit/hpcproftt -g {0}".format(profile_db_path)
        p = Popen(cmd, shell=True, stdout=PIPE, stderr=PIPE)
        msg, err = p.communicate()
        for line in msg.decode().split("\n"):
            if line.find("ctx id:") == -1 or line.find("value:") == -1: continue
            parts = line.split()
            node_id = int(parts[2][:-1])
            idx = 4
            while idx < len(parts):
                value = float(parts[idx][:-1])
                metric_id = parts[idx+3][:-1]
                if metric_id in self.metric_names:
                    if node_id not in self.metric_data:
                        self.metric_data[node_id] = {}
                    self.metric_data[node_id][metric_id] = value
                idx += 5
    ...
    def parse_xml_children(self, xml_node, hnode, callpath):
        ...
        """
        nid = hnode.node_dict['nid']
        if  nid in self.metric_data:
            for metric_id, val in self.metric_data[nid].items():
                hnode.add_metric(metric_id, val)
